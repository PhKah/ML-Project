...
# ============================================================================
# [KNOWLEDGE-DRIVEN] CONFIGURATION - MODELING WITH ENRICHED DATA
# ============================================================================
CONFIG = {
    'paths': {
...
print("=" * 80)
print("TASK 04: [REFAC] MODEL SELECTION ON KNOWLEDGE-ENRICHED DATA")
print("=" * 80)
...
def prepare_and_hide_test(config):
    print(f"\n[1] Loading Enriched Data & Hiding Test Set...")
    df = pd.read_csv(config['paths']['input_data'])
    
    # Identify indicators to ensure they are present
...
def find_best_threshold(model, X_val, y_val):
    """
    Tìm ngưỡng T tối ưu dựa trên F-beta (beta=0.5) để ưu tiên Precision.
    """
    if not hasattr(model, "predict_proba"):
        return 0.5, fbeta_score(y_val, model.predict(X_val), beta=0.5)
    
    y_probs = model.predict_proba(X_val)[:, 1]
    thresholds = np.linspace(0.01, 0.99, 99)
    
    f_beta_scores = [fbeta_score(y_val, y_probs >= t, beta=0.5, zero_division=0) for t in thresholds]
...
def save_winner(results, config):
    # Select winner based on Validation F0.5
    ...
    print(f"\n✓ WINNER LOCKED (F-beta): {winner_name}")
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 80)
    print("VALIDATION SUMMARY (F-BETA OPTIMIZED)")
    print("=" * 80)
