# Task [STT]: [Tên Nhiệm Vụ]

## 1. Mục tiêu & Bối cảnh
*   **Mục tiêu:** (Ví dụ: Thực hiện EDA để kiểm chứng Nghịch lý lựa chọn)
*   **Giai đoạn:** (Đối chiếu với `plan.md` - GĐ1/GĐ2/...)
*   **Giả thuyết/Câu hỏi:** (Ví dụ: Tỷ lệ match giảm khi số lượng lựa chọn tăng?)

## 2. Đầu vào & Đầu ra (Input/Output)
*   **Đầu vào:** (Đường dẫn file dữ liệu, ví dụ: `Data/data_final.csv`)
*   **Mã nguồn:** (Đường dẫn file code thực thi trong `src/`, ví dụ: `src/01_task_name.py`)
*   **Đầu ra:** (Đường dẫn file log, model, biểu đồ được tạo ra...)

## 3. Chiến lược thực hiện (Strategy)
*   **Dữ liệu sử dụng:** (Các biến từ `bare_minimum.md` liên quan)
*   **Phương pháp:** (Mô tả thuật toán, loại biểu đồ hoặc phép kiểm định sẽ dùng)
*   **Tiêu chuẩn thành công:** (Ví dụ: P-value < 0.05 hoặc F1-score > 0.6)
...
*   *(Ghi lại quá trình thực hiện thực tế, các lỗi gặp phải và cách sửa, hoặc các điều chỉnh so với kế hoạch ban đầu)*
